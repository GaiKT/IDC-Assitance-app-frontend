...
export default function CheckRoomTemp() {
    ...
    const onSubmit = async (data) => {
        try {
            setIsLoading(true);
            await axios.post('http://localhost:4000/checklists', { name: 'checklistroomtemp', formData: { ...data, user_id: user.user_id } });
            navigate('/');
            Toast.fire({
                icon: 'success',
                ...
                <div className="p-4 border flex flex-wrap rounded gap-4 justify-center">
                    {rackNames.map((rack, index) => (
                        <label key={index} className="flex flex-col items-center gap-5 border py-5 w-80 rounded">
                            Rack {rack}
                            <input {...register(`tempdetector_${index + 1}_temp`, { required: { value: true, message: "Temperature is required" }, max:{value:50 , message:"Vaule max 30"} , min:{value:5 , message:"Vaule min 5"}})} className="bg-gray-50 px-2" placeholder="Temp" />
                            {errors[`tempdetector_${index + 1}_temp`] && <span className="text-red-500">{errors[`tempdetector_${index + 1}_temp`]?.message}</span>}
                            <input {...register(`tempdetector_${index + 1}_hum`, { required: { value: true, message: "humidity is required" }, max:{value:100 , message:"Vaule max 100"} , min:{value:10 , message:"Vaule min 10"}})} className="bg-gray-50 px-2" placeholder="Hum" />
                ...
                <div className='flex w-full flex-col border rounded py-2 items-center'>
                    ...
                    <div className='flex p-4 gap-5 flex-wrap px-11'>
                        <label className='flex flex-col items-center gap-5 border py-5 w-80 rounded'>
                            สภาพอากาศภายนอก
                            <input {...register("outside_temp", { required: { value: true, message: "outside temp check is required" }, max:{value:100 , message:"Vaule max 100"} , min:{value:5 , message:"Vaule min 5"} })} className='bg-gray-50 px-2' placeholder='Temp'/>
                            {errors["outside_temp"] && <span className="text-red-500">{errors["outside_temp"]?.message}</span>}
                            <input {...register("outside_hum", { required: { value: true, message: "outside temp check is required"}, max:{value:100 , message:"Vaule max 100"} , min:{value:5 , message:"Vaule min 5"}})} className='bg-gray-50 px-2' placeholder='Hum'/>
                            {errors["outside_hum"] && <span className="text-red-500">{errors["outside_hum"]?.message}</span>}
                        </label>
                        <label className='flex flex-col items-center gap-5 border py-5 w-80 rounded'>
                            อุณหภูมิห้องหม้อแปลง
                            <input {...register("tr_room_temp", { required: { value : true , message:"Temperatrue check is required"}})} className='bg-gray-50 px-2' placeholder='Temp'/>
                            {errors["tr_room_temp"] && <span className="text-red-500">{errors["tr_room_temp"]?.message}</span>}
                            <input {...register("tr_room_hum", { required: { value : true , message:"humidity check is required"}})} className='bg-gray-50 px-2' placeholder='Hum'/>
