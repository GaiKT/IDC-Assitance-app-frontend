                ...
                <div className="border grid grid-cols-3 gap-2 p-2 max-md:grid-cols-1">
                    {rackNames.map((rack, index) => (
                        <label key={index} className="flex flex-col items-center gap-5 border py-2 rounded">
                            Rack {rack}
                            <input disabled={inputStatus} type='number' step='any' {...register(`TempDetector_${index + 1}_temp`, { required: { value: true, message: "Temperature is required" }, max:{value:50 , message:"Vaule max 30"} , min:{value:5 , message:"Vaule min 5"}})} className="bg-gray-50 px-2" placeholder="Temp" />
                            {errors[`TempDetector_${index + 1}_temp`] && <span className="text-red-500">{errors[`TempDetector_${index + 1}_temp`]?.message}</span>}
                            <input disabled={inputStatus} type='number' step='any' {...register(`TempDetector_${index + 1}_hum`, { required: { value: true, message: "humidity is required" }, max:{value:100 , message:"Vaule max 100"} , min:{value:10 , message:"Vaule min 10"}})} className="bg-gray-50 px-2" placeholder="Hum" />
                ...
                <div className='flex w-full flex-col border rounded py-2'>
                    ...
                    <div className='grid grid-cols-3 max-md:grid-cols-1 gap-2 px-2'>
                        <label className='flex flex-col items-center gap-5 border py-5 rounded'>
                            สภาพอากาศภายนอก
                            <input disabled={inputStatus}  type='number' step='any'  {...register("outside_temp", { required: { value: true, message: "outside temp check is required" }, max:{value:100 , message:"Vaule max 100"} , min:{value:5 , message:"Vaule min 5"} })} className='bg-gray-50 px-2' placeholder='Temp'/>
                            {errors["outside_temp"] && <span className="text-red-500">{errors["outside_temp"]?.message}</span>}
                            <input disabled={inputStatus}  type='number' step='any' {...register("outside_hum", { required: { value: true, message: "outside temp check is required"}, max:{value:100 , message:"Vaule max 100"} , min:{value:5 , message:"Vaule min 5"}})} className='bg-gray-50 px-2' placeholder='Hum'/>
                            {errors["outside_hum"] && <span className="text-red-500">{errors["outside_hum"]?.message}</span>}
                        </label>
                        <label className='flex flex-col items-center gap-5 border py-5 rounded'>
                            อุณหภูมิห้องหม้อแปลง
                            <input disabled={inputStatus} type='number' step='any' {...register("tr_room_temp", { required: { value : true , message:"Temperatrue check is required"}})} className='bg-gray-50 px-2' placeholder='Temp'/>
                            {errors["tr_room_temp"] && <span className="text-red-500">{errors["tr_room_temp"]?.message}</span>}
                            <input disabled={inputStatus} type='number' step='any' {...register("tr_room_hum", { required: { value : true , message:"humidity check is required"}})} className='bg-gray-50 px-2' placeholder='Hum'/>
                            {errors["tr_room_hum"] && <span className="text-red-500">{errors["tr_room_hum"]?.message}</span>}
                        </label>
                        <label className='flex flex-col items-center gap-5 border py-5 rounded'>
                            อุณหภูมิหม้อแปลงไฟฟ้า
                            <input disabled={inputStatus} type='number' step='any' {...register("tr_ch1", { required: { value : true , message : "Temperatrue is require"} , max : {value: 100, message: "Vaule max 100"}, min : {value: 0, message: "Value min 0"} })} className='bg-gray-50 px-2' placeholder='Temp CH1'/>
                            {errors["tr_ch1"] && <span className="text-red-500">{errors["tr_ch1"]?.message}</span>}
                            <input disabled={inputStatus} type='number' step='any' {...register("tr_ch2", { required: { value : true , message : "Temperatrue is require"} , max : {value: 100, message: "Vaule max 100"}, min : {value: 0, message: "Value min 0"} })} className='bg-gray-50 px-2' placeholder='Temp CH2'/>
                            {errors["tr_ch2"] && <span className="text-red-500">{errors["tr_ch2"]?.message}</span>}
                            <input disabled={inputStatus} type='number' step='any' {...register("tr_ch3", { required: { value : true , message : "Temperatrue is require"} , max : {value: 100, message: "Vaule max 100"}, min : {value: 0, message: "Value min 0"} })} className='bg-gray-50 px-2' placeholder='Temp CH3'/>
